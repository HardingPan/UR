...
from ss.submodules_RAFT_extractor256 import BasicEncoder256
from ss.submodules_RAFT_GRU256 import BasicUpdateBlock256
...
class RAFT256_SS(nn.Module):
    """
    RAFT
    """
    def __init__(self,args):
        super(RAFT256_SS,self).__init__()
    ...
    def deform(self, flow, n_iter=10):
        """ calc deformation via scaling and squaring w.r.t. flow """

        batch_num, _, ht, wd = flow.shape
        # When using matrix indexing(ij), meshgrid(M,N) yield output of shape (M,N),
        # where M,N refer to rows and columns respectively.
        # In Cartesion coordinates, x->column and y->row.
        y, x = torch.meshgrid(torch.arange(ht).to(flow.device), torch.arange(wd).to(flow.device), indexing='ij')
        # reshape x,y to (batch,1,H,W)
        x = x.repeat(batch_num,1,1)
        y = y.repeat(batch_num,1,1)
        ...
        for iter in range(n_iter):
            new_dx = self.remap(dx, x+dx, y+dy) + dx
            new_dy = self.remap(dy, x+dx, y+dy) + dy
            dx, dy = new_dx, new_dy
            
        return dx, dy

    def warping(self, flow):
          dx,dy = self.deform(flow)
          dxy = torch.stack((dx,dy),dim=1)

          assert dxy.shape == flow.shape, 'dxy wrong shape!'
          return dxy
    
    def forward(self,input,flowl0,args,flow_init=None,evaluate=False):
        ...
        coords0, _ = self.initialize_flow(img1)
        flow = torch.zeros_like(coords0)
        warp = torch.zeros_like(coords0)
        ...
        for itr in range(args.iters):
            ...
            warp = self.warping(flow)

            if args.upsample == 'convex':
                flow_up = self.upsample_flow(flow, up_mask)
            elif args.upsample == 'bicubic':
                flow_up = self.upsample_bicubic(self.upsample_bicubic(self.upsample_bicubic(flow)))
            elif args.upsample == 'bicubic8':
                flow_up = self.upsample_bicubic8(flow)
            elif args.upsample == 'lanczos4':
                B_f, C_f, H_f, W_f = coords1.shape
                flow_up = self.upsample_lanczos2_1(flow, new_size=[B_f, C_f, H_f * 2, W_f * 2])
                ...
                B_f, C_f, H_f, W_f = coords1.shape
                flow_up = self.upsample_lanczos8(flow, new_size=[B_f, C_f, H_f * 8, W_f * 8])
            else:
                raise ValueError('Selected upsample method not supported: ', args.upsample)
            
            if not evaluate:
                flow_up = self.warping(flow_up)
            else:
                pass
